 ...
 * Fetches gyroscope and accelerometer data from registers
 * Makes no use of the FIFO
...
// Define ACCEL_SENS and GYRO_SENS while we're at it
#define ACCEL_SENS 4096
#define GYRO_SENS 32.8
const uint8_t VAL_GYRO_CONFIG1 = 0b00000000;
const uint8_t VAL_ACCEL_CONFIG1 = 0b00000000;
...
/**
 * @brief   Reads n bytes starting from the specified register
 * 
...
/**
 * @brief   Writes n bytes starting from the specified register
 * 
 * @param[in]       reg_addr     Register to begin read/write at
 * @param[in]       len          Number of registers to read from sequentially
 * @param[in,out]   buf          Either used to store the read values, or filled by the user to contain values to be written
